...
class TasklistItemDelegate : public QItemDelegate
{
    ...
    protected:
        // Unfortunately, the QListWidgetItem has no 'checked()' or 'toggled()' signals. So we must filter the events
        // check if the click is within the checkbox area and emit the 'taskToggled()' signal manually.
        bool editorEvent(QEvent* ev, QAbstractItemModel* model, const QStyleOptionViewItem& option, const QModelIndex& idx);
